...
var isForApple = String( aThroughD.splice(0, 1) );
...
var bestFruit = String( aThroughD.slice(0, 1) );
...
for( var key in anakinSkywalker ){
  if( key.toString().startsWith( "former" ) ){
...
//15. Write a function called numChecker that takes in a parameter and checks to see if it is a number or a
  ...
  function numChecker( param ){
    return typeof parseInt( param ) === "number" && !isNaN(param);
  }
